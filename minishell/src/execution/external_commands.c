...
// static char	*check_and_return_path(const char *cmd, char **all_paths)
// {
// 	char	*temp;
// 	char	*path_to_search;
// 	char	*final_path;
...
// static char *find_path_in_env(char *cmd)
// {
//     char **all_paths;
//     char *path;
//     int    i;
...
//     i = 0;
//     all_paths = get_paths_from_env(g_data.envp);
//     path = NULL;
...
// static char *get_command_path(t_token *token)
// {
//     if (token->value[0] == '/' || token->value[0] == '.')
//         return (ft_strdup(token->value));  // Absolute or relative path
//     return (find_path_in_env(token->value));  // Look in PATH
// }
...
// 			exit(EXIT_FAILURE);
//     path = get_command_path(token);
//     if (!path)
// 			exit(EXIT_FAILURE);
//     if (execve(path, &token->value, g_data.envp) == -1)
//     {
//         free(path);
// 		exit(EXIT_FAILURE);
//     }
