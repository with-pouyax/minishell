...
/*   Updated: 2025/01/30 23:13:37 by pouyax           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				process_word(t_shell_data *shell, char *input, int *i, \
t_command *cmd);
...
void			exec_external_child(t_shell_data *shell, char *cmd_path, \
char **argv);
char			**allocate_and_fill_argv1(t_shell_data *shell, \
t_command *cmds, int *token_count);
...
char **arr_token, char *cmd_path);
int				process_heredoc_delimiter(t_shell_data *shell, char *input, \
int *i, t_token *heredoc_token);
int				read_heredoc_content(t_shell_data *shell, \
t_redirection *redir);
...
int *var_not_found_flag);
int				process_input_segment(t_shell_data *shell, int *i, \
int *cmd_index, t_command **last_cmd);
...
int				set_pipes(t_shell_data *shell, t_redirection *redir, \
int cmds_index);
int				has_redirs(t_redirection *redir, t_redirection_type type);
int				open_all_files(t_shell_data *shell, t_redirection *redir);
int				open_input_file(t_shell_data *shell, t_redirection *redir, \
int fd_in);
int				open_output_file(t_shell_data *shell, t_redirection *redir, \
int fd_out);
int				open_append_file(t_shell_data *shell, t_redirection *redir, \
int fd_out);
...
int				handle_syntax_error_s(t_shell_data *shell, \
t_redirection *new_redir, const char *unexpected_token);
int				handle_missing_filename_error(t_shell_data *shell, \
t_redirection *new_redir);
int				process_filename_or_delimiter( t_shell_data *shell, \
char *input, int *i, t_redirection *new_redir );
int				handle_heredoc_redirection( t_shell_data *shell, \
t_redirection *new_redir, char *filename_or_delimiter );
int				initialize_expansion(t_shell_data *shell, char **result);
int				handle_dollar(t_shell_data *shell, char *input, int *i, \
char **result);
int				toggle_quotes_and_append(t_shell_data *shell, \
char current_char, int *i, char **result);
int				handle_cd_minus(t_shell_data *shell);
int				initialize_conversion(const char **str, int *sign);
void			print_exit_error2(char *format, char *arg, int *exit_status, \
int code);
...
void			copy_existing_envp(char **new_envp, char **envp, \
int current_size);
int				collect_and_expand_word(t_shell_data *shell, char *input, \
int *i, char **expanded_word);
int				add_tokens_to_command(t_command *cmd, \
...
char *original_word);
int				validate_expanded_word(t_shell_data *shell, \
char *expanded_word);
...
int				finalize_redirection(t_shell_data *shell, \
t_redirection *new_redir);
int				handle_greater_operator(t_shell_data *shell, \
char *input, int *i, t_redirection *new_redir);
...
void			toggle_quotes_and_skip(t_shell_data *shell, \
char current_char, int *i);
int				handle_alpha_or_underscore(t_shell_data *shell, \
char *input, int *i, char **result);
int				handle_question_mark(t_shell_data *shell, char **result, \
int *i);
...
int				fork_and_execute(t_shell_data *shell, t_command *cmds, \
t_token *token);
